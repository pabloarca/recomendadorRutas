...
},{"tinyqueue":21}],12:[function(require,module,exports){
'use strict';
...
},{"./compactor":10,"./round-coord":14,"./topology":15,"@turf/distance":1,"turf-point":22}],14:[function(require,module,exports){
...
},{"./round-coord":14,"@turf/explode":2}],16:[function(require,module,exports){
...
},{"ieee754":16}],21:[function(require,module,exports){
'use strict';
...
if (!mapboxgl.supported()) {
  ...
  */
  ...
  const start_input = document.getElementById('map__start'),
    ...
    router = document.getElementById('map__action'),
    datalist = document.getElementById('map__places'),
    ...
    modal = document.getElementById('map__modal');
  ...
  fetch(new Request(url_base)).then(response => {
    ...
    path_network = new PathFinder(data, {
      precision: 1e-3,
    ...
    fetch(new Request(url_poi)).then(response => {
      ...
      poi_data
        ...
        .forEach(feature => {
          ...
          option['value'] = feature['properties']['nombre'] + ' (' + feature['properties']['tipo'] + ')';
  ...
  */
  ...
  [start_input, end_input].forEach(user_input => {
    user_input.addEventListener('change', e => {
      let value = e['target']['value'].split(' (')[0];
      if (user_input['id'].includes('start')) {
        selected_points['start'] = value;
      }
      else {
        selected_points['end'] = value
      }
      poi_route['features'] = poi_data.filter(feature => {
        return (feature['properties']['nombre'] === selected_points['start'] || feature['properties']['nombre'] === selected_points['end'])
      });
      map.getSource('start-end').setData(poi_route);
      map.getSource('path').setData(empty_line);
    });
  })

  router.addEventListener('click', () => {
    if (selected_points['start'] != undefined && selected_points['end'] != undefined && path_network != undefined) {
      let start = poi_data.filter(feature => feature['properties']['nombre'] === selected_points['start'])[0],
        end = poi_data.filter(feature => feature['properties']['nombre'] === selected_points['end'])[0],
        path = {
          'type': 'Feature',
          'properties': {},
          'geometry': {
      ...
      try {
        path['geometry']['coordinates'] = path_network.findPath(start, end)['path'];
      } catch (error) {
        modal.classList.toggle('map__modal--visible');
      }
  ...
  modal.addEventListener('click', () => modal.classList.toggle('map__modal--visible'))
}

},{"geobuf":9,"geojson-path-finder":12,"mapbox-gl":17,"moment":19,"moment/locale/es":18,"pbf":20}]},{},[23]);
